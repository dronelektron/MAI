...
int main(void)
{
	double a1 = 0.4, b1 = 1.0;
	double a2 = -1.0, b2 = 0.0;
	double eps = 0.000001;
	...
	printf("+-----------+----------+---------------+-----------------------+------------+-----------+----------+\n");
	printf("|     1     | [0.4, 1] |    Ньютона    |         0.6533        |%.9f |     -     |%.7f |\n", d1, n1);
	printf("+-----------+----------+---------------+-----------------------+------------+-----------+----------+\n");
	printf("|     2     | [-1, 0]  |   Дихотомии   |        -0.2877        |%.9f|%.8f|%.7f|\n", d2, i2, n2);
	printf("+-----------+----------+---------------+-----------------------+------------+-----------+----------+\n");
...
double f2_it(double x)
{
	return (log(-sqrt(1.0 + exp(2.0 * x)) + 2.0));
}

double f2_pr(double x)
{
	return (exp(x) + 2.0 * exp(2.0 * x) / (2.0 * sqrt(1.0 + exp(2.0 * x))));
}
